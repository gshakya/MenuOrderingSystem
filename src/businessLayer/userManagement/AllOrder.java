...
public class AllOrder implements SendToDB {
	...
	/*
	 * @Override public ArrayList<OrderOperation> runSelectQuery(String query) {
	 * Connection connect = ConnectToDB.getConnection(); Statement statement =
	 * null; ResultSet resultSet = null; ArrayList<OrderOperation> totOrder =
	 * new ArrayList<>(); try { statement = (Statement)
	 * connect.createStatement();
	 * 
	 * resultSet = statement.executeQuery(query);
	 * 
	 * while (resultSet.next()) { OrderOperation tmpOrder = new
	 * OrderOperation(); tmpOrder.setCount(resultSet.getInt("COUNT"));
	 * tmpOrder.setDishName(resultSet.getString("DISHNAME"));
	 * totOrder.add(tmpOrder); } } catch (SQLException e) { // TODO
	...
	public int getOrders(String dish) {
		...
		String query = "SELECT count(*) COUNT from orders A join dishes B on a.DISH_ID=b.DISH_ID WHERE A.CURRENT='Y' AND B.DISH_NAME='"
				+ dish + "' GROUP BY DISH_NAME ";
	...
	public void resetOrders() {
		String query = "UPDATE ORDERS SET CURRENT ='N' ";
		runInsertQuery(query);
	}
